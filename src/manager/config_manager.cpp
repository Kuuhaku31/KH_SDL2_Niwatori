...
bool
ConfigManager::Load_level_config(const std::string& file_path)
{
...
bool
ConfigManager::Load_game_config(const char* file_path)
{
    std::stringstream str_stream;
...
void
ConfigManager::parse_basic_temlpate(BasicTemplate& basic_template, cJSON* json_root)
{
...
void
ConfigManager::parse_player_template(PlayerTemplate& player_template, cJSON* json_root)
{
    if(!json_root || json_root->type != cJSON_Object) // 如果为空或者不是对象
    {
        return;
...
void
ConfigManager::parse_number_array(double* array, int max_len, cJSON* json_root)
{
...
void
ConfigManager::parse_tower_template(TowerTemplate& tower_template, cJSON* json_root)
{
    if(!json_root || json_root->type != cJSON_Object) // 如果为空或者不是对象
    {
        return;
...
void
ConfigManager::parse_enemy_template(EnemyTemplate& enemy_template, cJSON* json_root)
{
    if(!json_root || json_root->type != cJSON_Object) // 如果为空或者不是对象
    {
        return;
