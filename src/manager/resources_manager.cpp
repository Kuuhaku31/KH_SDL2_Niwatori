...
bool
ResourcesManager::load_from_file(SDL_Renderer* renderer)
{
    ...
    for(const auto& pair : texture_pool) // 遍历池里的所有配对
    {
        if(!pair.second) return false; // 如果一个配对的第二个元素失败
    }
...
const ResourcesManager::FontPool&
ResourcesManager::get_font_pool()
{
    return font_pool;
}

const ResourcesManager::SoundPool&
ResourcesManager::get_sound_pool()
{
    return sound_pool;
}

const ResourcesManager::MusicPool&
ResourcesManager::get_music_pool()
{
    return music_pool;
}

const ResourcesManager::TexturePool&
ResourcesManager::get_texture_pool()
{
    return texture_pool;
}
